...
while tentativas == True:
    ...
    if novo_produto in produtos:
        print("Produto já cadastrado na lista!")
        tentar = input("\nDeseja tentar novamente? (1 - sim)/(2 - não): ")
        if (tentar == 2) or (tentar == "não") or (tentar == "nao"):
            tentativas = False
    else:
        produtos.append(novo_produto)
        print("{} cadastrado com sucesso".format(novo_produto))
        print("\nSegue lista de produtos atualizada:")
...
while tentativas == True:
    prod = input("\nDigite o nome produto que deseja saber o valor: ")
    prod = prod.casefold()
    if prod in produtos:
        i_produto = produtos.index(prod)
        print ("O preço do {} é R${}".format(prod,precos[i_produto]))
        n_tentativa = input("\nDeseja tentar novamente? (1 - sim)/(2 - não): ")
        if (n_tentativa == 2) or (n_tentativa == "não") or (n_tentativa == "nao"):
            tentativas = False
            print("OK, finalizamos por aqui então")
        else:
            print('OK, vamos lá:\n')
    else:
        print("{} não está na lista de produtos!".format(prod))
